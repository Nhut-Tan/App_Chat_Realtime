...
class PrivateChat{
    ...
    function get_all_chat_data(){
        $query="
        SELECT a.user_name as from_user_name, b.user_name as to_user_name, chat_message,timestamp, to_user_id, from_user_id
        FROM chat_message JOIN users a on chat_message.from_user_id=a.user_id
                        JOIN users b on chat_message.to_user_id=b.user_id
    ...
    function change_chat_status(){
        ...
        UPDATE chat_message
        SET status='Yes'
        WHERE from_user_id=:from_user_id AND to_user_id=:to_user_id AND status='No'
        ";
        $statement = $this->connect->prepare($query);
		$statement->bindParam(':from_user_id', $this->from_user_id);
        $statement->bindParam(':to_user_id', $this->to_user_id);
